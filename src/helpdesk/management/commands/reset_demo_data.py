...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not noinput:
            confirm = input(
                _("This will DELETE ALL helpdesk data and reset to demo state. "
    ...
    def _clear_attachment_files(self, keep_attachments=False):
        ...
        self.stdout.write(_("  Clearing attachment files (preserving demo fixtures)..."))
    ...
    def _load_demo_fixture(self):
        ...
        if demo_fixture is None:
            ...
            candidate = os.path.join(project_root, 'demodesk', 'fixtures', 'demo.json')
            if os.path.exists(candidate):
                demo_fixture = candidate

        if demo_fixture is None:
            raise FileNotFoundError(
                _("Demo fixture 'demo.json' not found in FIXTURE_DIRS or project root.")
            )
    ...
    def _verify_data(self):
        ...
        for queue_name in expected_queues:
            if queue_name not in queues:
                self.stdout.write(
                    self.style.WARNING(
                        _("  Missing expected queue: %s") % queue_name
                    )
                )
        ...
        if not admin_user:
            self.stdout.write(
                self.style.WARNING(_("  Admin user not found!"))
            )
